import React from "react";
import './App.module.css';
import styles from './App.module.css' //모듈 스타일 import


function App() {
  return (
      <div>
        <div className={styles['frame1']}>
          <div className={styles['group16']}>
            <div className={styles['navi']}>
            <span className={styles['text']}>
              <span>진미님 힘내세요</span>
            </span>
            </div>
          </div>
          <div>
            <div className={styles['search']}>
              <div className={styles['searchbar']}>
                <div className={styles['searchbar1']}>
                  <div className={styles['frame']}>
                    <img
                        src="/playground_assets/searchicon1251-pj.svg"
                        alt="Searchicon1251"
                        className={styles['searchicon']}
                    />
                  </div>
                </div>
              </div>
              <button className={styles['searchbutton']}>
              <span className={styles['text02']}>
                <span>search</span>
              </span>
              </button>
            </div>
          </div>
          <div className={styles['tag']}>
          <span className={styles['text04']}>
            <span>운동</span>
          </span>
          </div>
          <div className={styles['tag1']}>
          <span className={styles['text06']}>
            <span>뷰티</span>
          </span>
          </div>
          <div className={styles['tag2']}>
          <span className={styles['text08']}>
            <span>공부</span>
          </span>
          </div>
          <div className={styles['tag3']}>
          <span className={styles['text10']}>
            <span>Books</span>
          </span>
          </div>
          <div className={styles['tag4']}>
          <span className={styles['text12']}>
            <span>업무</span>
          </span>
          </div>
          <div className={styles['tag5']}>
          <span className={styles['text14']}>
            <span>Articles</span>
          </span>
          </div>
          <div className={styles['frame7']}>
            <div className={styles['empty']}>
              <span className={styles['text16']}>+</span>
            </div>
            <div>
              <div className={styles['frameframe']}>
                <div className={styles['frame01']}>
                  <img
                      src="/playground_assets/ellipse73214-1enj-200h.png"
                      alt="Ellipse73214"
                      className={styles['ellipse7']}
                  />
                  <img
                      src="/playground_assets/ellipse83214-2v7l-200h.png"
                      alt="Ellipse83214"
                      className={styles['ellipse8']}
                  />
                  <img
                      src="/playground_assets/ellipse93214-e4qd-200h.png"
                      alt="Ellipse93214"
                      className={styles['ellipse9']}
                  />
                </div>
                <div className={styles['frame02']}>
                  <img
                      src="/playground_assets/edit13216-vtq-200h.png"
                      alt="edit13216"
                      className={styles['edit1']}
                  />
                  <img
                      src="/playground_assets/close13216-kxv-200h.png"
                      alt="close13216"
                      className={styles['close1']}
                  />
                </div>
              </div>
              <div className={styles['frame03']}>
                <div className={styles['frame04']}>
                  <div className={styles['frame05']}>
                  <span className={styles['text17']}>
                    <span>연구개발보고서 쓰기...</span>
                  </span>
                  </div>
                  <div className={styles['frame06']}>
                  <span className={styles['text19']}>
                    <span>2021.09.24</span>
                  </span>
                  </div>
                </div>
              </div>
              <div className={styles['frame07']}>
                <div className={styles['frame08']}>
                <span className={styles['text21']}>
                  <span>비고</span>
                </span>
                </div>
              </div>
              <div className={styles['frame09']}>
                <img
                    src="/playground_assets/fire13233-0nka-200w.png"
                    alt="fire13233"
                    className={styles['fire1']}
                />
                <div className={styles['tag6']}>
                <span className={styles['text23']}>
                  <span>업무</span>
                </span>
                </div>
              </div>
            </div>
            <div className={styles['frame2']}>
              <div className={styles['frameframe1']}>
                <div className={styles['frame10']}>
                  <img
                      src="/playground_assets/ellipse73217-7cga-200h.png"
                      alt="Ellipse73217"
                      className={styles['ellipse71']}
                  />
                  <img
                      src="/playground_assets/ellipse83217-1kcxi-200h.png"
                      alt="Ellipse83217"
                      className={styles['ellipse81']}
                  />
                  <img
                      src="/playground_assets/ellipse93217-jv9-200h.png"
                      alt="Ellipse93217"
                      className={styles['ellipse91']}
                  />
                </div>
                <div className={styles['frame11']} className={styles['frame11']}>
                  <img
                      src="/playground_assets/edit13217-kavd-200h.png"
                      alt="edit13217"
                      className={styles['edit11']}
                  />
                  <img
                      src="/playground_assets/close13217-5fps-200h.png"
                      alt="close13217"
                      className={styles['close11']}
                  />
                </div>
              </div>
              <div>
                <div className={styles['frame13']}>
                  <div className={styles['frame14']}>
                  <span className={styles['text25']}>
                    <span>연구개발보고서 쓰기...</span>
                  </span>
                  </div>
                  <div className={styles['frame15']}>
                  <span className={styles['text27']}>
                    <span>2021.09.24</span>
                  </span>
                  </div>
                </div>
              </div>
              <div className={styles['frame16']}>
                <div className={styles['frame17']}>
                <span className={styles['text29']}>
                  <span>중요한 Task로써 열심히 해야합ㄴ디ㅏ.</span>
                </span>
                </div>
              </div>
              <div className={styles['frame18']}>
                <img
                    src="/playground_assets/fire13234-wsq-200w.png"
                    alt="fire13234"
                    className={styles['fire11']}
                />
              </div>
            </div>
            <div className={styles['frame3']}>
              <div className={styles['framecontainer']}>
                <div className={styles['frameframe2']}>
                  <div
                      className={styles['frame19']}
                      className={styles['frame19']}
                  >
                    <img
                        src="/playground_assets/ellipse73218-ypf-200h.png"
                        alt="Ellipse73218"
                        className={styles['ellipse72']}
                    />
                    <img
                        src="/playground_assets/ellipse83218-71zc-200h.png"
                        alt="Ellipse83218"
                        className={styles['ellipse82']}
                    />
                    <img
                        src="/playground_assets/ellipse93218-iy6u-200h.png"
                        alt="Ellipse93218"
                        className={styles['ellipse92']}
                    />
                  </div>
                  <div className={styles['frame20']}>
                    <img
                        src="/playground_assets/edit13218-9ibd-200h.png"
                        alt="edit13218"
                        className={styles['edit12']}
                    />
                    <img
                        src="/playground_assets/close13218-4857-200h.png"
                        alt="close13218"
                        className={styles['close12']}
                    />
                  </div>
                </div>
                <div>
                  <div className={styles['frame22']}>
                    <div className={styles['frame23']}>
                    <span className={styles['text31']}>
                      <span>연구개발보고서 쓰기...</span>
                    </span>
                    </div>
                    <div className={styles['frame24']}>
                    <span className={styles['text33']}>
                      <span>2021.09.24</span>
                    </span>
                    </div>
                  </div>
                </div>
                <div className={styles['frame25']}>
                  <div className={styles['frame26']}>
                  <span className={styles['text35']}>
                    <span>중요한 Task로써 열심히 해야합ㄴ디ㅏ.</span>
                  </span>
                  </div>
                </div>
                <div className={styles['frame27']}>
                  <img
                      src="/playground_assets/fire13235-3abq-200w.png"
                      alt="fire13235"
                      className={styles['fire12']}
                  />
                </div>
              </div>
            </div>
          </div>
          <div
              className={styles['frame21']}
              className={styles['frame21']}
              className={styles['frame21']}
          >
            <div className={styles['empty1']}>
              <span className={styles['text37']}>+</span>
            </div>
            <div className={styles['frame12']} className={styles['frame12']}>
              <div className={styles['frameframe3']}>
                <div className={styles['frame28']}>
                  <img
                      src="/playground_assets/ellipse73219-8qc4-200h.png"
                      alt="Ellipse73219"
                      className={styles['ellipse73']}
                  />
                  <img
                      src="/playground_assets/ellipse83219-regf-200h.png"
                      alt="Ellipse83219"
                      className={styles['ellipse83']}
                  />
                  <img
                      src="/playground_assets/ellipse93219-9wio-200h.png"
                      alt="Ellipse93219"
                      className={styles['ellipse93']}
                  />
                </div>
                <div className={styles['frame29']}>
                  <img
                      src="/playground_assets/edit13219-m6u-200h.png"
                      alt="edit13219"
                      className={styles['edit13']}
                  />
                  <img
                      src="/playground_assets/close13219-82g-200h.png"
                      alt="close13219"
                      className={styles['close13']}
                  />
                </div>
              </div>
              <div className={styles['frame30']}>
                <div>
                  <div className={styles['frame32']}>
                  <span className={styles['text38']}>
                    <span>질문 제목입니다 내용이 길어...</span>
                  </span>
                  </div>
                  <div className={styles['frame33']}>
                  <span className={styles['text40']}>
                    <span>2021.09.24</span>
                  </span>
                  </div>
                </div>
              </div>
              <div className={styles['frame34']}>
                <div className={styles['frame35']}>
                <span className={styles['text42']}>
                  <span>
                    <span>
                      제가 언어를 처음배우는데, 이렇게 코드를 작성했더니 오류가
                      나요. 왜이러는 걸까요??
                    </span>
                    <br></br>
                    <span>
                      내용이 길어지는경우는 이렇게 처리하면 될 것 같...
                    </span>
                  </span>
                </span>
                </div>
              </div>
              <div className={styles['frame36']}>
                <div className={styles['tag7']}>
                <span className={styles['text47']}>
                  <span>뷰티</span>
                </span>
                </div>
              </div>
            </div>
            <div>
              <div className={styles['frameframe4']}>
                <div className={styles['frame37']}>
                  <img
                      src="/playground_assets/ellipse73221-vb2-200h.png"
                      alt="Ellipse73221"
                      className={styles['ellipse74']}
                  />
                  <img
                      src="/playground_assets/ellipse83221-3a4e-200h.png"
                      alt="Ellipse83221"
                      className={styles['ellipse84']}
                  />
                  <img
                      src="/playground_assets/ellipse93221-6k8e-200h.png"
                      alt="Ellipse93221"
                      className={styles['ellipse94']}
                  />
                </div>
                <div className={styles['frame38']}>
                  <img
                      src="/playground_assets/edit13221-7jua-200h.png"
                      alt="edit13221"
                      className={styles['edit14']}
                  />
                  <img
                      src="/playground_assets/close13221-gb7k-200h.png"
                      alt="close13221"
                      className={styles['close14']}
                  />
                </div>
              </div>
              <div className={styles['frame39']}>
                <div className={styles['frame40']}>
                  <div className={styles['frame41']}>
                  <span className={styles['text49']}>
                    <span>질문 제목입니다 내용이 길어...</span>
                  </span>
                  </div>
                  <div className={styles['frame42']}>
                  <span className={styles['text51']}>
                    <span>2021.09.24</span>
                  </span>
                  </div>
                </div>
              </div>
              <div className={styles['frame43']}>
                <div className={styles['frame44']}>
                <span className={styles['text53']}>
                  <span>
                    <span>
                      제가 언어를 처음배우는데, 이렇게 코드를 작성했더니 오류가
                      나요. 왜이러는 걸까요??
                    </span>
                    <br></br>
                    <span>
                      내용이 길어지는경우는 이렇게 처리하면 될 것 같...
                    </span>
                  </span>
                </span>
                </div>
              </div>
              <div className={styles['frame45']}></div>
            </div>
            <div className={styles['frame31']} className={styles['frame31']}>
              <div className={styles['framecontainer1']}>
                <div className={styles['frameframe5']}>
                  <div className={styles['frame46']}>
                    <img
                        src="/playground_assets/ellipse73222-rgr-200h.png"
                        alt="Ellipse73222"
                        className={styles['ellipse75']}
                    />
                    <img
                        src="/playground_assets/ellipse83222-4tm5x-200h.png"
                        alt="Ellipse83222"
                        className={styles['ellipse85']}
                    />
                    <img
                        src="/playground_assets/ellipse93222-29t-200h.png"
                        alt="Ellipse93222"
                        className={styles['ellipse95']}
                    />
                  </div>
                  <div className={styles['frame47']}>
                    <img
                        src="/playground_assets/edit13222-83tl-200h.png"
                        alt="edit13222"
                        className={styles['edit15']}
                    />
                    <img
                        src="/playground_assets/close13222-fvg-200h.png"
                        alt="close13222"
                        className={styles['close15']}
                    />
                  </div>
                </div>
                <div className={styles['frame48']}>
                  <div className={styles['frame49']}>
                    <div className={styles['frame50']}>
                    <span className={styles['text58']}>
                      <span>질문 제목입니다 내용이 길어...</span>
                    </span>
                    </div>
                    <div className={styles['frame51']}>
                    <span className={styles['text60']}>
                      <span>2021.09.24</span>
                    </span>
                    </div>
                  </div>
                </div>
                <div className={styles['frame52']}>
                  <div className={styles['frame53']}>
                  <span className={styles['text62']}>
                    <span>
                      <span>
                        제가 언어를 처음배우는데, 이렇게 코드를 작성했더니
                        오류가 나요. 왜이러는 걸까요??
                      </span>
                      <br></br>
                      <span>
                        내용이 길어지는경우는 이렇게 처리하면 될 것 같...
                      </span>
                    </span>
                  </span>
                  </div>
                </div>
                <div className={styles['frame54']}></div>
              </div>
            </div>
          </div>
        </div>
      </div>
  );
}

export default App;
